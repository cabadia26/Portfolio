...
'I am a full-stack software developer driven by my passion for the positive impact of technology. My background in healthcare provides a unique perspective, enabling me to excel in all aspects of software development, from requirements gathering to database implementation, middle-tier development, and front-end design, with a strong focus on delivering top-quality software with a user-centric approach.
', 1
...
insert DevSubsection(DevSectionId, DevSubsectionCode, DevSubsectionName, DevSubsectionDesc, GitHubRepoName, URL, DevSubsectionSequence)
    select  
...
union select  
    ...
    'Portfolio', 'https://ca-portfolio.azurewebsites.net', 15
union select  
    ...
    'RecordKeeper', 'https://recordkeeperwebsite.azurewebsites.net', 16
...
union select  
    ...
    'Recipe','https://recipewebsitecpu.azurewebsites.net',  17
--union select  
...
union select  x.DevSubsectionId, 'wireframe', 'Wireframe', 'img', '.png', 1 from x  where x.DevSectionCode = 'desktopapp' --and (x.DevSubsectionName <>'tictactoe')
...
from DevSubsection s 
cross join DevTool t 
where s.DevSubsectionCode in ( 'tictactoe', 'flipacoin')  and t.DevtoolName in ('C#', 'Git', 'GitHub', 'Wire Framing', 'Desktop Apps', 'Cloud Computing')
...
union 
select s.DevSubsectionId, 'count of book', 'select count(*) from Book', 0 from DevSubsection s where s.DevSubsectionCode = 'BookstoreDB'
union 
select s.DevSubsectionId, 'list book', 'select * from Book', 1 from DevSubsection s where s.DevSubsectionCode = 'BookstoreDB'
union 
select s.DevSubsectionId, 'count of recipe', 'select count(*) from Recipe', 0 from DevSubsection s where s.DevSubsectionCode = 'RecipeDB'
union 
select s.DevSubsectionId, 'list recipe', 'select * from recipe', 1 from DevSubsection s where s.DevSubsectionCode = 'RecipeDB'
